...
export default function Home({bild, attDisplera}) {
    ...
    const hanteraSubmit =  async event => {
        ...
        axios(config)
            .then(function (response) {
                router.push(`/admin/${state.vara}/${state.swish}/${state.pris+"€"+state.datum+"€"+state.kategori}`)
                setByt(false)
            })
    ...
    return byt === false
        ? (
            ...
            <>
                <Head>
                    <title>Ändra kvitto för {attDisplera.vara}</title>
                    <meta name="description" content="Hemsida för att se inlagda kvitton."/>
                    <link rel="apple-touch-icon" sizes="180x180" href="/apple-touch-icon.png"/>
                    <link rel="icon" type="image/png" sizes="32x32" href="/favicon-32x32.png"/>
                    <link rel="icon" type="image/png" sizes="16x16" href="/favicon-16x16.png"/>
...
export async function getStaticProps({params}) {
    ...
    const data = {
        vara: params.vara, pris: pris, swish: params.swish, datum: datum, kategori: kategori
    }
    return {
        props: {
